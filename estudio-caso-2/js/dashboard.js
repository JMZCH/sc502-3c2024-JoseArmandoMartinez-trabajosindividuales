document.addEventListener('DOMContentLoaded', function () {
    ...
    const COMMENT_API_URL = "backend/comment.php";
    ...
    async function loadTasks() {
        try {
            ...
            if (!tasksResponse.ok) {
                if (tasksResponse.status == 401) window.location.href = "index.html";
            ...
            const commentsResponse = await fetch(COMMENT_API_URL, {
                method: 'GET',
                credentials: 'include'
            });
            ...
            tasks.forEach(task => {
                task.comments = allComments.filter(comment => comment.task_id == task.id);
            });
            ...
            if (task.comments && task.comments.length > 0) {
                ...
                task.comments.forEach(comment => {
                    ...
                    <li class="list-group-item">
                        ${comment.description} 
                        <button type="button" 
                                class="btn btn-sm btn-link remove-comment" 
